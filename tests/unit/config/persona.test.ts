...
describe('PersonaAgent', () => {
  ...
  it('should call searchFAQs → generateResponse → logQuery → logResponse and return message + confidence + category', async () => {
    ...
    mockGenerateResponse.mockResolvedValue({ response: 'Node is a runtime.', confidence: 0.85 });

    const result = await agent.processQuery('learner-123', 'What is Node?');

    expect(mockSearchFAQs).toHaveBeenCalledWith('What is Node?');
    expect(mockGenerateResponse).toHaveBeenCalledWith('What is Node?', fakeFAQs);
    expect(mockLogQuery).toHaveBeenCalledWith('learner-123', 'What is Node?');
    expect(mockLogResponse).toHaveBeenCalledWith('learner-123', 'What is Node?', 'Node is a runtime.', 0.85, 'tech');

    expect(result).toEqual({ message: 'Node is a runtime.', confidence: 0.85, category: 'tech' });
  });
  ...
  it('should return graceful apology message with confidence 0 if searchFAQs throws error', async () => {
    ...
    const result = await agent.processQuery('learner-123', 'Test question');

    expect(result).toEqual({ message: 'I apologize, but I encountered an error. Please try again.', confidence: 0 });
